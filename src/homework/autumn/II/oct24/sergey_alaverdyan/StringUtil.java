package homework.autumn.II.oct24.sergey_alaverdyan;
...
public class StringUtil {
    ...
    public static String trim(String inputText) {
          int first = 0;
        String str = "";
        int last = inputText.length() - 1;
        while (inputText.charAt(first) == ' ') {
            first++;
        }
        while (inputText.charAt(last) == ' ') {
            last--;
        }
        for (int i = first; i <= last; i++) {

            str = str + inputText.charAt(i);
